...
class Front_Wheels(object):
	...
	def __init__(self, debug=False, db="config", bus_number=1, channel=FRONT_WHEEL_CHANNEL):
		...
		self._angle_value = self._straight_angle
	...
	def turn_straight(self):
		...
		self.angle = self._straight_angle
	...
	def angle(self, new_angle):
		# sanity check to protect the hardware
		if new_angle < self._min_angle:
			new_angle = self._min_angle
		if new_angle > self._max_angle:
			new_angle = self._max_angle
		self._angle_value = new_angle
		self.wheel.write(self._angle_value)
